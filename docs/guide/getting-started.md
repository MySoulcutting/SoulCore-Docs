...
SoulCore 的 Fabric 客户端可以独立提供战斗文字、怪物血条、拾取提示和设置界面；配合 SoulCore Paper 插件可使用物品图片、怪物血条名称规则与服务端效果，配合 CustomQuest 则可使用任务对话、导航和追踪 HUD。
...
> GeckoLib 和专用 ModernUI Text Port 已内嵌在 Fabric Mod 中，**不需要**再单独下载。请从 `mods/` 移除完整 ModernUI-MC（mod ID `modernui`），它与 SoulCore 的内嵌端口明确冲突；已有其他 GeckoLib 版本时也应确认兼容性。
...
4. 启动 Minecraft 客户端（需通过 Fabric Loader 启动）。Mod 会自动创建本地资源目录：
   ```
...
将支持 SoulCore `1.4.0` 客户端通道的 CustomQuest 放入服务端 `plugins/`。任务对话、导航和追踪快照由 CustomQuest 直接发送给 Fabric 客户端，**不依赖 SoulCore Paper 插件**。
...
继续阅读：[安装详解](/guide/installation) · [平滑字体](/guide/modules/smooth-fonts) · [CustomQuest 任务对话](/guide/modules/quest-dialogue) · [自定义物品图片](/guide/modules/item-images)
